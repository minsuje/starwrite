...
def lambda_handler(event, context):
    ...
    # 검색 결과 처리 및 문서 객체로 변환
    question = "웹툰에 대해서 알려줘"
    search_results = neo4j_vector_search(question)
    documents = [
        Document(result["text"], metadata={"score": result["score"]})
        for result in search_results
    ]

    # LangChain 체인 생성 및 질의 처리
    neo4j_vector_store = Neo4jVector.from_existing_graph(
        embedding=OpenAIEmbeddings(),
        url=NEO4J_URI,
        username=NEO4J_USERNAME,
    ...
    chain = RetrievalQAWithSourcesChain(
        model=ChatOpenAI(temperature=0, api_key=OPENAI_API_KEY), retriever=retriever
    )

    response = chain(
        {"question": question, "documents": documents}, return_only_outputs=True
    )
